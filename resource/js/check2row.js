...
function showtable(data, trafficitem)
{
    ...
    table+='<th class="text-center">報到</th><th class="text-center">姓名(錄取編號)/學校</th><th class="text-center">去程交通</th><th class="text-center">登錄</th>';
    table+='</tr></thead>';    
    ...
    for(i=0;i<data.length;i++)
    {
        ...
        table+='<tr>';
        if (data[i]['checkin']==1){
            table+='<td rowspan="2" class="text-center alert alert-success col-md-2" style="vertical-align: middle;">是</td>';            
        }else{
            table+='<td rowspan="2" class="text-center alert alert-danger  col-md-2" style="vertical-align: middle;">否</td>';
        }


        table+='<td class="text-center col-md-3" style="vertical-align: middle;">'+data[i]['name']+' - '+data[i]['sn']+'</td>';
        //table+='<td>'+data[i]['cp']+'</td>';
        //table+='<td class="text-center" style="vertical-align: middle;">'+data[i]['school']+'</td>';
        
        realgo=data[i]['realgo'];
        table+='<td  rowspan="2" style="vertical-align: middle;" class="col-md-4"><select class="form-control input-sm trafficitem-go col-md-4" id="gotraffic'+idx+'" idx="'+idx+'" odata="'+realgo+'" ogo="'+data[i]['go']+'">';//+data[i]['realgo']+'</td>';
        ...
        table+='<td class="text-center col-md-2" style="vertical-align: middle;"><button type="button" class="btn btn-success checkin" idx="'+idx+'">報到</button></td>';
        ...
        table+='</tr>';
        table+='<tr><td class="text-center col-md-3" style="vertical-align: middle;">'+data[i]['school']+'</td>';
        table+='<td class="text-center col-md-2" style="vertical-align: middle;"><button type="button" class="btn btn-danger checkout" idx="'+idx+'">取消</button></td></tr>';
    }    
    table+='</tbody>'; 
