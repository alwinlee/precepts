<?php
   ...
   //------------------------------------------------------------------------------------------------------------------------------
    ...
    // set column title
    $xlstitle=array("學員姓名","學員代號","性別","學員班級","報到序號","區域","報名宗仰", "報到");
    $xlstitleW=array(18,18,8,20,12,20,12,12);
    ...
    $area=array("全區","北區","桃區","竹區","中區","嘉區","園區","南區","高區","海外","淨人");
    for($nSheet=0;$nSheet<count($area);$nSheet++){
        ...
        if ($nSheet==0){
            ...
            $sql="select * from `".$tbname."` where (`area`='".$area[$nSheet]."') order by `id` ASC";
        }
       ...
       //middleAlignment($objWorkSheet,$item,$sql);
        ...
        while($row=mysql_fetch_array($result,MYSQL_ASSOC))//MYSQL_NUM))//MYSQL_ASSOC))
        {
            ...
            $idx++;$currR++;$c=0;
            $objWorkSheet->setCellValue($col[$c].$currR,$row["name"])
                         ->setCellValue($col[++$c].$currR,$row["studentid"])//$row["ARE_ID"])
                         ->setCellValue($col[++$c].$currR,$row["sex"])
                         ->setCellValue($col[++$c].$currR,$row["classroom"])
                         ->setCellValue($col[++$c].$currR,$row["serial"])
                         ->setCellValue($col[++$c].$currR,$row["area"])
                         ->setCellValue($col[++$c].$currR,$row["register"]==1?"1":"")
