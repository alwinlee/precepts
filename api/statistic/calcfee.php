...
*/
    ...
    while($row=mysql_fetch_array($record, MYSQL_ASSOC))
    {
        ...
        if(($row["go"]!=$row["realgo"])||($row["back"]!=$row["realback"]))
        {
            $cost=$realgofee+$realbackfee;
            if($row["pay"]<($realgofee+$realbackfee)){//(paid-(realgofee+realbackfee))<0
                $diff=($realgofee+$realbackfee)-$row["pay"];//+:補費用
            }
        }else{
            $cost=$gofee+$backfee;
            if(($gofee+$backfee)==($row["pay"]*2)){
                $diff=($gofee+$backfee)*-1;//-:退費用
            }else if ($row["pay"]<($gofee+$backfee)){
                $diff=(($gofee+$backfee)-$row["pay"]);//+:補費用
            }
        }
        if($diff>0){$command[]="update `student` set `lack`=".$diff.",`over`=0,`cost`=".$cost." where `id`=".$row["id"];}
        else if($diff<0){$diff*=-1;$command[]="update `student` set `over`=".$diff.",`lack`=0,`cost`=".$cost." where `id`=".$row["id"];}
        else{$command[]="update `student` set `lack`=0,`over`=0,`cost`=".$cost." where `id`=".$row["id"];}
    }
