<?php
  ...
  // parameters
  $commandstring = $_POST["command"];
  $datestring = $_POST["date"];
  $monthstring = $_POST["month"];
  $timestring = $_POST["time"];
  if (!$commandstring) {
    $commandstring = "`studentid`='TW00103008' OR `studentid`='TW00104039' OR `studentid`='TW00126975'";
  }
  if (!$datestring) {
    $datestring = "xx月xx日(六)";
  }
  ...
  $info=getPDFinfo($datestring, $monthstring, $timestring);
  $html_line=getPDFLn();
  $spLine=getPDFSPLn();
  ...
  $sub = $commandstring;//"`studentid`='TW00103008' OR `studentid`='TW00104039' OR `studentid`='TW00126975'";
  $sql="SELECT * FROM `".$tbname."` WHERE (".$sub.") order by `classroom` asc, `sex` desc, `studentid` asc;";
